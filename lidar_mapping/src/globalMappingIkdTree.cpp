...
KD_TREE<PointT>::Ptr kdtree_ptr(new KD_TREE<PointT>()); //(new KD_TREE<PointT>(0.3, 0.2, 0.6))
KD_TREE<PointT> &ikd_Tree = *kdtree_ptr;
...
public:
    LaserMapping(ros::NodeHandle &n) : is_initial(false), update_count(0), key(0)
    {
        ...
        odom_sub_ = new message_filters::Subscriber<nav_msgs::Odometry>(nh, "/local_odom", 5);      
        laser_sub_ = new message_filters::Subscriber<sensor_msgs::PointCloud2>(nh, "/local_map", 5); 
        ...
        pubLaserLocalizer = nh.advertise<nav_msgs::Odometry>("/laser_localizer", 5, false);
        pubLaserLocalizerCloud = nh.advertise<sensor_msgs::PointCloud2>("/laser_localizer_cloud", 5, false);
        pubMapCloud = nh.advertise<sensor_msgs::PointCloud2>("/map_data_cloud", 5, false);
        pubGraphNode = nh.advertise<visualization_msgs::Marker>("/graph_nodes", 5, false);
    ...
    void combineCallback(const nav_msgs::Odometry::ConstPtr odom, const sensor_msgs::PointCloud2::ConstPtr laser)
    {
        ...
        else if (floor <= 2.0 && floor > -5.0)
        {
            floor_id = 0;
        }

        else if (floor <= 5.0 && floor > 2.0)
        {
            floor_id = 1;
        }
    ...
    void InsertPoints(pcl::PointCloud<PointT>::Ptr cloud)
    {
        ...
        ikd_Tree.Add_Points(cloud->points, false);
    }
    void ApproxNearestNeighbors(pcl::PointCloud<PointT>::Ptr que, pcl::PointCloud<PointT>::Ptr ref)
    {
        ...
        for (int i = 0; i < copy->points.size(); ++i)
        {
            ...
            ikd_Tree.Nearest_Search(copy->points[i], 5, search_result, pointSearchSqDis, 1.5); //Radius_Search
            for (int j = 0; j < search_result.size(); ++j)
            {
                // bool point_selected = search_result.size() < 5 ? false : pointSearchSqDis[4] > 5 ? false
                //                                                                                  : true;
                // if (point_selected)
                // {
                ref->points.push_back(search_result[j]);
                // }
            }

            // PointVector search_result;
            // ikd_Tree.Radius_Search(copy->points[i], 0.5, search_result);
            // for (int j = 0; j < search_result.size(); ++j)
            // {
            //     PointT point = search_result[j];
            //     ref->points.push_back(point);
            // }
        }

        cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << ref->points.size() << endl;
    }
    ...
    void pubMapData()
    {
        ...
        map_data->clear();
        map_data->points = ikd_Tree.PCL_Storage;
        mtx.unlock();
    ...
    void pubLowerFreq()
    {
        ...
        factor_info.scale.x = 1; //2
        factor_info.scale.y = 1;
        factor_info.scale.z = 1;
        ...
        factor_info.pose.position.z = values.at<gtsam::Pose3>(size - 1).translation().z() + 10; //+15
        // factor_info.pose.position.x = 0;
        // factor_info.pose.position.y = 30;
        // factor_info.pose.position.z = 15;
...
int main(int argc, char **argv)
{
    ...
    std::thread visualizeMapThread(&LaserMapping::visualizeGlobalMapThread, &lm);
    ros::spin();
