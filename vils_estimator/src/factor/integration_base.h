...
class IntegrationBase
{
  public:
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
      ...
      //add-on  
    void init_refine_delta_pvq_bgs (const Vector3d dbg){
        Eigen::Matrix3d dp_dbg = jacobian.block<3, 3>(O_P, O_BG);
        Eigen::Matrix3d dq_dbg = jacobian.block<3, 3>(O_R, O_BG);
        Eigen::Matrix3d dv_dbg = jacobian.block<3, 3>(O_V, O_BG);
        delta_q = delta_q * Utility::deltaQ(dq_dbg * dbg);
        delta_v = delta_v  + dv_dbg * dbg;
        delta_p = delta_p  + dp_dbg * dbg;	
    }
    void init_refine_delta_pvq_bas (const Vector3d dba){
        Eigen::Matrix3d dp_dba = jacobian.block<3, 3>(O_P, O_BA);
        Eigen::Matrix3d dv_dba = jacobian.block<3, 3>(O_V, O_BA);
        delta_v = delta_v  + dv_dba * dba;
